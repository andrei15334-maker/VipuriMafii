    ...
    // ─── LIDER SETTINGS: AFISARE MEMBRII ───
      ...
      try {
        ...
        const list = members.map(m => {
          let badge = '👤 Membru';
          // Check Discord leader roles first (most up-to-date), then fall back to DB ownerId
          const hasLeaderRole = liderRoleIds.some(rid => m.roles.cache.has(rid));
          if (m.id === mafia.ownerId || hasLeaderRole) {
            badge = '👑 Lider';
